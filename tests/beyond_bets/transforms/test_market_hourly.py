...
def test_market_hourly_transformation(sample_bets_data):
    """
    The MarketHourly transform should group the bets by market and hour, 
    and sum the bet_amount for each hour.
    """
    mh = MarketHourly()
    
    # Override the randomly generated bets data
    mh._inputs["bets"] = SampleDataset(sample_bets_data)
